        ...
        if resp.status_code == 200:
            ...
            for zone_id, zone in data.items():
                rows.append({
                    ...
                    "Name": zone.get("name"),
                    "Type": zone.get("type"),
                    "Scene": zone.get("scene")
                })
            if rows:
                return pd.DataFrame(rows)
            return pd.DataFrame(columns=["Zone ID", "Name", "Type", "Scene"])
...
def get_sessions():
    try:
        ...
        if resp.status_code == 200:
            ...
            rows = []
            for session in data:
                for zone_id, count in session.get("zone_visit_counts", {}).items():
                    rows.append({
                        "Person": session.get("object_id"),
                        "Zone ID": zone_id,
                        "Entries": count
                    })
            if rows:
                return pd.DataFrame(rows)
            return pd.DataFrame(columns=["Person", "Zone ID", "Entries"])
        return pd.DataFrame([{"Error": f"API returned {resp.status_code}"}])
    except Exception as e:
        return pd.DataFrame([{"Error": str(e)}])
...
with gr.Blocks(title="Storewide Loss Prevention Dashboard", css="footer {display: none !important;}") as demo:
    gr.HTML(HEADER_HTML)
    ...
    with gr.Row():
        with gr.Column():
            ...
            zones_table = gr.Dataframe(value=get_zones(), interactive=False)
        with gr.Column():
            gr.Markdown("## Region Entry Stats (Person/Zone)")
            sessions_table = gr.Dataframe(value=get_sessions(), interactive=False)

    gr.Markdown("---")
    gr.Markdown("## Alert Summary (by Type)")
    alert_summary_table = gr.Dataframe(value=get_alert_summary(), interactive=False)

    gr.Markdown("---")
    gr.Markdown("## All Alerts")
    alerts_table = gr.Dataframe(value=get_alerts(), interactive=False)
