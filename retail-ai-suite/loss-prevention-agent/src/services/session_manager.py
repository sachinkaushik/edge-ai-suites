...
class SessionManager:
    ...
    async def on_scene_data(
        ...
        Updates session liveness (last_seen), cameras, bbox, and region
        membership snapshot.  Does NOT fire ENTERED/EXITED events — those
        come from on_region_event() via SceneScape's native region events.
        """
        ...
        for obj in objects:
            ...
            # Extract current region IDs from the snapshot (for display only)
            regions_raw = obj.get("regions", {})
            current_regions: Set[str] = set()
            if isinstance(regions_raw, dict):
                current_regions = set(regions_raw.keys())
            elif isinstance(regions_raw, list):
                for r in regions_raw:
                    if isinstance(r, dict):
                        current_regions.add(r.get("id", r.get("region_id", "")))
                    else:
                        current_regions.add(str(r))
            current_regions.discard("")
            ...
            if oid in self._sessions:
                ...
                session.current_regions = set(current_regions)
                session.bbox = bbox
                ...
                logger.info("Session created", object_id=oid, regions=list(current_regions))
    ...
    async def _expire_session(self, oid: str) -> None:
        session = self._sessions.pop(oid, None)
        if session is None:
            return
    ...
    async def _fire_enter(
        ...
        # Increment per-region visit counter
        session.visit_count_per_region[region_id] = (
            session.visit_count_per_region.get(region_id, 0) + 1
        )
